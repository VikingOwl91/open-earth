...
/* MapLibre line rendering can be only a couple of pixels wide. Query a forgiving screen-space box instead of requiring a pixel-perfect click. */
...
/* Keep normal layer handlers, plus a ~9 px click/hover tolerance for tectonic steps and faults. */
for(const id of TECTONIC_LAYERS){map.on('mouseenter',id,()=>map.getCanvas().style.cursor='pointer');map.on('mouseleave',id,()=>map.getCanvas().style.cursor='')}
map.on('click',e=>{const direct=map.queryRenderedFeatures(e.point,{layers:TECTONIC_LAYERS.filter(id=>map.getLayer(id))});if(direct.length)return;const tectonic=nearbyRenderedFeature(e.point,TECTONIC_LAYERS,10);if(tectonic){showTectonicDetail(tectonic);return}if(viewState.layers.faults&&map.getLayer('faults')){const fault=nearbyRenderedFeature(e.point,['faults'],11);if(fault)selectFeature('faults',fault)}});
map.on('mousemove',e=>{const tectonic=nearbyRenderedFeature(e.point,TECTONIC_LAYERS,8),fault=viewState.layers.faults&&map.getLayer('faults')?nearbyRenderedFeature(e.point,['faults'],9):null;if(tectonic||fault)map.getCanvas().style.cursor='pointer'});
for(const id of TECTONIC_LAYERS)map.on('click',id,e=>{const f=e.features?.[0];if(f)showTectonicDetail(f)});
