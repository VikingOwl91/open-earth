...
(() => {
  ...
  function renderVolcano(f,tab=activeTab){activeTab=tab;const incoming=f.properties||{},x=catalogProps(incoming),p=x.p,coords=(x.geometry||f.geometry)?.coordinates||[],name=value(p.Volcano_Name,p.VolcanoName,p.Report_Name,'Volcano'),number=vid(p),has=!!reportForVolcano(p),body=document.querySelector('#details-body'),panel=document.querySelector('#details');selectedMapFeature={layer:f.properties?.Report_Name?'activity':'volcanoes',feature:f};let content='';if(tab==='history')content=`<section class="drawer-section"><div class="inspector-heading">Eruption history</div>${eventRows(p)}<a class="drawer-source-link" href="https://volcano.si.edu/volcano.cfm?vn=${encodeURIComponent(number)}&vtab=Eruptions" target="_blank" rel="noreferrer">View full history on GVP ↗</a></section>`;else if(tab==='nearby')content=coords.length>=2?relationships(coords):'';else if(tab==='sources')content=sources(p);else content=overview(p,coords);body.innerHTML=`<header class="drawer-head"><div><div class="eyebrow">Volcano${number?` · GVP ${esc(number)}`:''}</div><h2><span class="feature-icon volcano-icon" aria-hidden="true">▲</span>${esc(name)} ${has?'<em class="gvp-badge">ACTIVE · WEEKLY REPORT</em>':''}</h2></div></header>${tabs()}<div class="drawer-content">${content}</div>`;panel.hidden=false;wireDrawer(f,p);if(tab==='overview')loadPoiImage(name,p.Country)}
