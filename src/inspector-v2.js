...
(() => {
  ...
  function overview(p,coords){
    const report=reportForVolcano(p),type=value(p.Primary_Volcano_Type,p.PrimaryVolcanoType,p.Volcano_Type),elev=value(p.Elevation_m,p.Elevation),last=value(p.Last_Known_Eruption,p.LastKnownEruption),tect=value(p.Tectonic_Setting,p.TectonicSetting),country=value(p.Country,p.country);
    const plate=coords.length>=2&&typeof containingPlate==='function'?containingPlate(coords):null;
    const plateTxt=plate?plateName(plate):null;
    return `<div class="poi-hero" data-poi-image><div class="poi-image-placeholder">Loading image…</div></div>${reportCard(p)}<section class="drawer-section"><div class="inspector-heading">Basic information</div><div class="meta">${fact('Location',[country,coords.length>=2?`${(+coords[1]).toFixed(3)}, ${(+coords[0]).toFixed(3)}`:null].filter(Boolean).join(' · '))}${fact('Type',type)}${fact('Elevation',elev!=null?`${elev} m`:null)}${fact('Last known eruption',last)}${fact('Tectonic setting',tect)}${plateTxt?fact('PB2002 plate',plateTxt):''}</div></section>`;
  }
  ...
  function renderVolcano(f,tab=activeTab){activeTab=tab;const incoming=f.properties||{},x=catalogProps(incoming),p=x.p,coords=(x.geometry||f.geometry)?.coordinates||[],name=value(p.Volcano_Name,p.VolcanoName,p.Report_Name,'Volcano'),number=vid(p),has=!!reportForVolcano(p),body=document.querySelector('#details-body'),panel=document.querySelector('#details');selectedMapFeature={layer:f.properties?.Report_Name?'activity':'volcanoes',feature:f};let content='';if(tab==='history')content=`<section class="drawer-section"><div class="inspector-heading">Eruption history</div>${eventRows(p)}<a class="drawer-source-link" href="https://volcano.si.edu/volcano.cfm?vn=${encodeURIComponent(number)}&vtab=Eruptions" target="_blank" rel="noreferrer">View full history on GVP ↗</a></section>`;else if(tab==='nearby')content=coords.length>=2?relationships(coords):'';else if(tab==='sources')content=sources(p);else content=overview(p,coords);body.innerHTML=`${window.renderInspectorNavBar?.()||''}<header class="drawer-head"><div><div class="eyebrow">Volcano${number?` · GVP ${esc(number)}`:''}</div><h2><span class="feature-icon volcano-icon" aria-hidden="true">▲</span>${esc(name)} ${has?'<em class="gvp-badge">ACTIVE · WEEKLY REPORT</em>':''}</h2></div></header>${tabs()}<div class="drawer-content">${content}</div>`;window.wireInspectorNavBar?.(body);panel.hidden=false;wireDrawer(f,p);if(tab==='overview')loadPoiImage(name,p.Country)}
  ...
  function installSemanticMarkers(){
    if(!map.getStyle()||!map.getSource('volcanoes')||!map.getSource('activity'))return;
  ...
  async function load(){const d=await localSnapshot('eruptions.json','eruptions');eruptionData=d?.features?d:empty;if(map.getStyle())installAfterHydration()}
  load();
})();
